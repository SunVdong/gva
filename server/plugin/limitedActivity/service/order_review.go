...
// CreateReview 对已核销的活动订单发表评价（一单一评，仅本人）
func (s *orderReview) CreateReview(req laRequest.CreateOrderReviewRequest, userID uint) (model.OrderReview, error) {
...
// 使用硬删除：MySQL 下 order_id 唯一索引会包含软删行，软删会导致「删后重评」失败。
