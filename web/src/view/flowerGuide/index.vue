  ...
  <div>
    <div class="gva-search-box">
      ...
      >
        ...
        <el-form-item label="状态" prop="templateID">
            ...
            <div class="w-full">
              <el-form :model="previewForm" label-width="120px">
                ...
                <el-form-item label="默认排序">
                  <el-input
                    v-model="previewForm.order"
                    placeholder="例如: id desc"
                  />
                </el-form-item>
                ...
                <div v-if="previewConditions.length === 0" class="text-gray">
                  该模板暂无条件
                </div>
                  ...
                  <el-form-item :label="cond.column + ' ' + cond.operator">
                    <template v-if="cond.operator === 'BETWEEN'">
                      <div class="flex gap-2 w-full">
                        <el-input
                          v-model="previewForm['start' + cond.from]"
                          placeholder="开始值: start{{cond.from}}"
                        />
                        <el-input
                          v-model="previewForm['end' + cond.from]"
                          placeholder="结束值: end{{cond.from}}"
                        />
                      </div>
                    </template>
                    <template v-else>
                      <el-input
                        v-model="previewForm[cond.from]"
                        :placeholder="'变量: ' + cond.from"
                      />
                    </template>
                  </el-form-item>
                </template>
...
const openPreview = async (row) => {
  ...
  if (res.code === 0) {
    ...
    previewConditions.value = (previewTemplate.value.conditions || []).map(
      (c) => ({
        from: c.from,
        column: c.column,
        operator: c.operator,
...
const runPreview = async () => {
  ...
  const res = await previewSQL({
    templateID: previewTemplate.value.templateID,
    params,
  });
  if (res.code === 0) {
